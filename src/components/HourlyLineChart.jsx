...
function HourlyLineChart({ hourlyData, showAll = false }) {
	...
	const displayData = showAll
		? hourlyData
		: hourlyData.filter((_, idx) => idx % 3 === 0);
	...
	const getWeatherEmoji = (conditionText) => {
		...
		// Rain conditions
			...
			condition.includes("violent rain")
		) {
			return "🌧️"; // Heavy rain cloud
		} else if (
			condition.includes("moderate rain") ||
			condition.includes("rain showers")
		) {
		...
		// Snow conditions
			...
			condition.includes("slight snow") ||
			condition.includes("snow grains")
		) {
			return "❄️";
	...
	useEffect(() => {
		...
		const xScale = (index) =>
			margin.left + (index / (displayData.length - 1)) * chartWidth;
		const yScale = (temp) =>
			margin.top + ((paddedMax - temp) / (paddedMax - paddedMin)) * chartHeight;
		...
		for (
			...
			i += Math.max(1, Math.floor(displayData.length / 6))
		) {
			const x = xScale(i);
			ctx.beginPath();
			...
			{hoveredPoint !== null && displayData[hoveredPoint] && (
				<div className="chart-tooltip">
					<div className="tooltip-content">
						<strong>
							{new Date(displayData[hoveredPoint].time).toLocaleTimeString(
								"en-US",
								{
									hour: "numeric",
									hour12: true,
